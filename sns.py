...
while True:
    ...
    for a in items:
        # 순위
        rank = a.select_one('em.ico.rank').text.strip()

        # 책 제목
        title = a.select_one('a.gd_name').text.strip()

        # 저자
        author_tag = a.select_one('span.authPub.info_auth a')
        author = author_tag.text.strip() if author_tag else ''

        # 출판사
        pub_tag = a.select_one('span.authPub.info_pub a')
...
print(f"\n총 {len(df)}권을 수집했습니다.")
print(df)
df.to_csv("SNS_20191526.csv",index=False,encoding="utf-8-sig")